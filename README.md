...
The PRNs journey reads the first page of PRNs awaiting acceptance from the backend and checks a matching rendered row for its number, material, issuer and tonnage. CI supplies a representative PRN through the backend-owned WireMock mapping. Deployed runs require at least one such PRN for the journey producer; an empty list fails explicitly. The check is read-only and uses the existing backend Basic/OAuth settings. The list API currently does not filter by obligation year; this assertion does not establish year filtering.
